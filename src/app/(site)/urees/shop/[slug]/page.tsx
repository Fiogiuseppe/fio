...
export async function generateStaticParams() {
  ...
  return getProductsByCategory('urees').map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const product = getProduct(slug);
  if (!product || product.category !== 'urees') return {};
  return {
    title: `${product.title} — UREES`,
  ...
  return (
    <article className="px-6 py-12 md:px-10 md:py-20">
      <div className="mx-auto mb-10 max-w-7xl">
        <Link href="/urees#shop" className="urees-site__back">
          ← UREES Shop
        </Link>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2 lg:gap-16">
        ...
        <div className="flex flex-col justify-center">
          ...
          <TypographyMeta className={editorial.stack.labelToTitle}>UREES · One of one</TypographyMeta>
